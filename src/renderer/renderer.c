...
int renderer_init(Renderer* renderer, SDL_WindowFlags window_flags)
{
    ...
    renderer->window = SDL_CreateWindow(renderer->name, 640, 480, window_flags);
...
Matrix4x4 Matrix4x4_create_orthographic_off_center(
	float left,
	float right,
	float bottom,
	float top,
...
Matrix4x4 Matrix4x4_create_perspective_field_of_view(
	float fieldOfView,
	float aspectRatio,
...
Matrix4x4 Matrix4x4_create_look_at(
